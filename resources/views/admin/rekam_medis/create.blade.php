            ...
            <form action="{{ route('rekam_medis.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="id_dokter" id="id_dokter" class="form-control @error('id_dokter') is-invalid @enderror">
                        ...
                        @else
                            @foreach ($data_dokter as $dokter)
                                <option value="{{ $dokter->id }}" {{ old('id_dokter') == $dokter->id ? 'selected' : '' }}>{{ $dokter->user->name }}</option>
                            @endforeach
                        @endif
                    </select>
                    @error('id_dokter')
                ...
                <div class="form-group">
                    ...
                    <select name="id_poliklinik" id="id_poliklinik" class="form-control @error('id_poliklinik') is-invalid @enderror" >
                        ...
                        @foreach($data_poliklinik as $poliklinik)
                            <option value="{{ $poliklinik->id }}" {{ old('id_poliklinik') == $poliklinik->id ? 'selected' : '' }}>{{ $poliklinik->nama }}</option>
                        @endforeach
                    </select>
                    @error('id_poliklinik')
                        <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group">
                    ...
                    <textarea name="diagnosa" id="diagnosa" class="form-control @error('diagnosa') is-invalid @enderror" rows="4" >{{ old('diagnosa') }}</textarea>
                    @error('diagnosa')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group">
                    ...
                    <textarea name="resep_obat" id="resep_obat" class="form-control @error('resep_obat') is-invalid @enderror" rows="4" >{{ old('resep_obat') }}</textarea>
                    @error('resep_obat')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
